...
在filebeat中主要配置的是prospectors，用于指定特定文件，并对其进行简单的加工。

对于prospectors，主要的配置是设置想要追踪的文件的路径，也可以配置可选，如定义输入类型和读取文件使用的编码，排除和包括指定线，添加自定义field，等等。
...
上面定义一个单一的prospector，用于追踪Apache 访问和错误日志，使用了通配符*来包含所有在apache2目录下的所有文件并添加一个field apache到消息中，注意prospectors是一个YAML数组，所以以-开始。

### filebeat processors

filebeat可以在发送到你指定的目的地之前，应用简单的处理和数据增强到日志数据。你可以解码JSON字符串，丢弃指定field添加多种元数据（如Docker，Kubernetes）等等。

processor在每个filebeat配置里的prospector定义，你可以使用条件状态应用你的处理，下面是一个drop_fields processor，对Apache访问日志丢弃一些fields



### filebeat 输出

 这个部分用于输出到你想输出的地方，支持许多输出选择，如控制台，文件，云，redis和kafka，但大多数情况下，你会使用logstash或者elasticsearch输出类型。



定义一个logstash 实例用于更多处理和数据增强。
...
此设置适用于当前正在运行filebeat进程。filebeat配置文件不会更改。
